...
```bash
claude mcp add obsidian -- npx obsidian-agent --mcp
```
...
```json
{
  "mcpServers": {
    "obsidian": {
      "command": "npx",
      "args": ["obsidian-agent", "--mcp", "--vault", "Work"]
    }
  }
}
```
...
Start the server once (`npx obsidian-agent`), then point clients at it. The HTTP
...
Get a key with `obsidian-agent keys new "Cursor"` or from the Settings page.
